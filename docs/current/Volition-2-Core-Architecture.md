...
- **Function:** Allows a Abe to dynamically modify GUPPI's `asyncio.gather` listener list to track project-specific streams (e.g., `volition:project_alpha`) without restarting the daemon.