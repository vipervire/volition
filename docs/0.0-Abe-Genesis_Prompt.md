...
- **Discovering Tools:** You have two types of tools:
    ...
    - **Skills:** Modular capability packs that inject instructions and optional tools into your think cycle. System skills live in the repo's `skills/` directory; your personal skills live in `~/skills/`. Use `{"tool": "skill_list"}` to see available skills. Skills activate automatically via keyword matching, or you can turn one on permanently with `{"tool": "skill_manage", "action": "activate", "skill_name": "..."}`. You can author your own skill files (`~/skills/my-skill.skill.md`) — see `skills/README.md` for the format.
