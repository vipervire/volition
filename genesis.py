...
def check_prerequisites():
    ...
    print("This script will generate the service files for the following. They default to using local Ollama for embeddings and Claude CLI for summarization. Ensure your local Ollama instance is running and reachable, and that the Claude CLI is installed and authenticated. You will be asked to setup/start these services later after setting up Redis:")
...
def main():
    ...
    print("\n[Claude CLI Config]")
    print("Volition now uses the Claude CLI (claude) as its LLM provider.")
    print("Ensure the Claude CLI is installed and authenticated before running the Matts.")
...
# --- Claude CLI Provider Config ---
CLAUDE_CLI={claude_cli}

# --- Model Routing (Split-Brain) ---
# Flash: Social, Low-Risk (Chat)
MODEL_FLASH={model_flash}
# Pro: Work, High-Agency (Inbox/Tasks)
MODEL_PRO={model_pro}

# Scribe / Summarization model
CLAUDE_MODEL={model_flash}
...
Environment=CLAUDE_CLI={claude_cli}
Environment=CLAUDE_MODEL_SUMMARIZE={model_flash}

# Ollama fallback (local summarization if preferred)
Environment=OLLAMA_URL=http://localhost:11434/api
Environment=MODEL_SUMMARIZE=mistral
...
apt-get install -y -qq python3-full python3-pip python3-venv git redis-tools curl nano nodejs npm
...
/root/venv/bin/pip install aiosqlite redis asyncssh aiohttp requests chromadb trafilatura
...
if ! command -v "$CLAUDE_BIN" &>/dev/null; then
    echo "[WARN] Claude CLI not found at '$CLAUDE_BIN'. You must authenticate manually after boot."
    echo "[WARN] Run: claude (then follow the login prompts)"
else
    echo "[OK] Claude CLI found at: $(which $CLAUDE_BIN)"
    echo "[ACTION REQUIRED] After first boot, run 'claude' inside the container to authenticate."
fi
...
echo ""
echo "IMPORTANT: Before starting guppi, authenticate the Claude CLI:"
echo "  pct enter {vmid}"
echo "  claude"
echo "  (follow the login prompts, then exit the container)"
echo "  systemctl start guppi"
"""
    ...
    print("\n4. Authenticate the Claude CLI inside the container (REQUIRED before starting guppi):")
    print(f"   {CYAN}pct enter {vmid}{RESET}")
    print(f"   {CYAN}claude{RESET}  # follow the login prompts")
    print(f"   {CYAN}exit{RESET}")

    print("\n5. Start the Matt:")
    print(f"   {CYAN}systemctl start guppi{RESET}")

    print("\nNotes:")
    print("- The Claude CLI must be authenticated inside the container before guppi will function.")
